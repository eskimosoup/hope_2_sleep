class ShippingOption < ActiveRecord::Base
  ...
  validates_presence_of :price, :region, :min_weight, :max_weight
  validates_format_of :price, :with => /^\d+(\.\d{1,2})?$/
	validates_numericality_of :min_weight, :only_integer => true, :greater_than_or_equal_to => 0
	validates_numericality_of :max_weight, :only_integer => true, :greater_than_or_equal_to => 0
  ...
  def self.find_by_country_and_weight(country, weight)
    weight = weight.to_i
    self.find_by_country(country).select{|x| weight <= x.max_weight && weight >= x.min_weight}
  end
